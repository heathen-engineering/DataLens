...
public:
    ...
    static bool WriteResultsToTextFile(const FString& OutputFilePath, const TArray<FDataLensTestResult>& Results)
    {
        ...
        for (const FDataLensTestResult& Result : Results)
        {
            ...
            Output += FString::Printf(TEXT("Rows: %lld | Cols: %lld | RowSpan: %lld bytes | TotalMem: %lld bytes | Duration: %.3f ms\n"),
        ...
        return FFileHelper::SaveStringToFile(Output, *OutputFilePath, FFileHelper::EEncodingOptions::AutoDetect, &IFileManager::Get(), FILEWRITE_None);
    }
    ...
    static bool WriteResultsToCSV(const FString& OutputFilePath, const TArray<FDataLensTestResult>& Results)
    {
        ...
        return FFileHelper::SaveStringToFile(CSV, *OutputFilePath, FFileHelper::EEncodingOptions::AutoDetect, &IFileManager::Get(), FILEWRITE_None);
    }
    ...
    static TArray<FDataLensTestResult> Test_Phase0_CreatePreAllocatedWithDataAndPadding()
    {
        ...
        {
            FDataLensTestResult Result;
            Result.TestName = FString::Printf(TEXT("Test_Phase0_CreatePreAllocatedWithDataAndPadding_%lldRows"), RowCount);
            Result.Timestamp = FDateTime::Now().ToString();
            ...
            try
            {
...

                {
                    ...
                    Notes = FString::Printf(TEXT("Created store with %lld loaded rows + %lld extra padded rows, %d columns, pre-generated data."),
                        RowCount, ExtraRows, (int)Result.Columns);
                } // Store destroyed here, memory freed
    ...
    static TArray<FDataLensTestResult> Test_Phase0_GetRaw()
    {
        ...
        for (int64 RowCount : StoreSizes)
        {
            ...
            for (int32 IterCount : Iterations)
            {
                ...
                try
                {
                    for (int32 i = 0; i < IterCount; ++i)
                    {
                        ...
                        case 0:
                        {
                            ...
                            if (Value != float(Row)) bSuccess = false;
                            break;
                        }
                        case 1:
                        {
                            int32 Value = Store.GetRaw<int32>(Row, Col);
                            if (Value != int32(Row)) bSuccess = false;
                            break;
                        }
                        case 2:
                        {
                            double Value = Store.GetRaw<double>(Row, Col);
                            if (Value != double(Row)) bSuccess = false;
                            break;
                        }
                        }
                    }
                ...
                Result.bSuccess = bSuccess;
                if (Notes.IsEmpty()) Notes = TEXT("Checked random accesses and verified correctness.");
            ...
            for (int32 IterCount : Iterations)
            {
                ...
                try
                {
                    for (int32 i = 0; i < IterCount; ++i)
                    {
                        ...
                        {
                            float Value{};
                            if (!Store.TryGet<float>(Row, Col, Value) || Value != float(Row)) bSuccess = false;
                            break;
                        }
                        case 1:
                        {
                            int32 Value{};
                            if (!Store.TryGet<int32>(Row, Col, Value) || Value != int32(Row)) bSuccess = false;
                            break;
                        }
                        case 2:
                        {
                            double Value{};
                            if (!Store.TryGet<double>(Row, Col, Value) || Value != double(Row)) bSuccess = false;
                            break;
                        }
                        }
                    }
                ...
                Result.bSuccess = bSuccess;
                if (Notes.IsEmpty()) Notes = TEXT("Checked random TryGet accesses and verified correctness.");
    ...
    static TArray<FDataLensTestResult> Test_Phase0_SetRaw()
    {
        ...
        for (int64 RowCount : StoreSizes)
        {
            ...
            struct CellUpdate { size_t Row; size_t Col; double DblVal; int32 IntVal; float FltVal; };
            ...
            for (int32 i = 0; i < MaxIters; ++i)
            {
                ...
                Updates.push_back({ Row, Col, double(Row * 1.1), int32(Row * 10), float(Row * 1.0f) });
            }
            ...
            for (int32 IterCount : Iterations)
            {
                ...
                try
                {
                    for (int32 i = 0; i < IterCount; ++i)
                    {
                        ...
                        switch (U.Col)
                        {
                        case 0: Store.SetRaw<float>(U.Row, U.Col, U.FltVal); break;
                        case 1: Store.SetRaw<int32>(U.Row, U.Col, U.IntVal); break;
                        case 2: Store.SetRaw<double>(U.Row, U.Col, U.DblVal); break;
                        }
                    }
                }
                catch (const std::exception& e)
                ...
                try
                {
                    for (int32 i = 0; i < IterCount; ++i)
                    {
                        ...
                        switch (U.Col)
                        {
                        case 0: bSuccess &= (Store.GetRaw<float>(U.Row, U.Col) == U.FltVal); break;
                        case 1: bSuccess &= (Store.GetRaw<int32>(U.Row, U.Col) == U.IntVal); break;
                        case 2: bSuccess &= (Store.GetRaw<double>(U.Row, U.Col) == U.DblVal); break;
                        }
                    }
                    if (Notes.IsEmpty()) Notes = TEXT("Performed SetRaw on random cells and verified values.");
                }
                catch (const std::exception& e)
                {
                    bSuccess = false;
            ...
            for (int32 IterCount : Iterations)
            {
                ...
                try
                {
                    for (int32 i = 0; i < IterCount; ++i)
                    {
                        ...
                        switch (U.Col)
                        {
                        case 0: bOk = Store.TrySet<float>(U.Row, U.Col, U.FltVal); break;
                        case 1: bOk = Store.TrySet<int32>(U.Row, U.Col, U.IntVal); break;
                        case 2: bOk = Store.TrySet<double>(U.Row, U.Col, U.DblVal); break;
                        }
                        bSuccess &= bOk;
                    }
                }
                ...
                try
                {
                    ...
                    if (Notes.IsEmpty()) Notes = TEXT("Performed TrySet on random cells and verified values.");
                }
                catch (const std::exception& e)
                {
                    bSuccess = false;
    ...
    static FDataLensTestResult Test_Phase0_TryGetResilience()
    {
        ...
        try
        {
            ...
            bSuccess &= Store.TryGet<float>(0, 2, fValue);   // column 2 is double, copying into float should safely copy only sizeof(float)
            bSuccess &= Store.TryGet<double>(0, 0, dValue);  // column 0 is float, copying into double should safely copy only sizeof(double)
        ...
        for (int64 Rows : RowCounts)
        {
            ...
            try
            {
                ...
                for (int64 r = 0; r < Rows; ++r)
                {
                    ...
                    std::memcpy(DataBuffer.data() + offset, &f, sizeof(f)); offset += sizeof(f);
                    std::memcpy(DataBuffer.data() + offset, &i, sizeof(i)); offset += sizeof(i);
                    std::memcpy(DataBuffer.data() + offset, &d, sizeof(d)); offset += sizeof(d);
                }
                ...
                else
                {
                    ...
                    for (int64 idx : SampleRows)
                    {
                        ...
                        float fVal; int32 iVal; double dVal;
                        std::memcpy(&fVal, Dumped.data() + offset, sizeof(fVal)); offset += sizeof(fVal);
                        std::memcpy(&iVal, Dumped.data() + offset, sizeof(iVal)); offset += sizeof(iVal);
                ...
                Notes += FString::Printf(TEXT("Loaded and dumped %lld rows, %d columns."), Rows, (int)Cols.size());
            }
            catch (const std::exception& e)
            {
                bSuccess = false;
