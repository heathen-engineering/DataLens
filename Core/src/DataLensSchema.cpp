...
DataLensValueType DataLensValueTypeUtils::SmallestUnsignedForMax(uint64_t maxValue)
{
	if (maxValue <= UINT8_MAX)  return DataLensValueType::UInt8;
	if (maxValue <= UINT16_MAX) return DataLensValueType::UInt16;
	if (maxValue <= UINT32_MAX) return DataLensValueType::UInt32;
	return DataLensValueType::UInt64;
}

DataLensValueType DataLensValueTypeUtils::SmallestSignedForRange(int64_t minValue, int64_t maxValue)
{
	if (minValue >= INT8_MIN  && maxValue <= INT8_MAX)  return DataLensValueType::Int8;
	if (minValue >= INT16_MIN && maxValue <= INT16_MAX) return DataLensValueType::Int16;
	if (minValue >= INT32_MIN && maxValue <= INT32_MAX) return DataLensValueType::Int32;
	return DataLensValueType::Int64;
}
