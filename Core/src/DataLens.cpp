...
DataLens::DataLens(const DataLensSchema& schema)
	: mSchema(schema)
{
	...
	for (size_t i = 0; i < mSchema.Count(); ++i)
	{
		...
		if (!store.Validate())
		{
			DATALENS_LOG(Error, ("DataStoreSchema '" + store.Name +
				"' is invalid. Ensure it contains at least 2 columns and the first column is a single byte named '" +
				std::string(DataLensRowFlagsName) + "'.").c_str());
	...
	if (!bIsValid)
	{
		DATALENS_LOG(Error, "DataLens failed to initialize one or more stores due to invalid schema.");
	}
}
...
bool DataLens::SetViewInsert(size_t id, const std::string sql)
{
    ...
    {
		// (diagnostic log removed during extraction)
        return false;
    }
...
QueryResultCache DataLens::RunQuery(const DataQueryObject& query)
{
	...
	for (size_t row = 0; row < rowCount; ++row)
	{
		...
		{
			if (!mStores[firstStoreIdx].IsValidRow(row)) // row liveness (validity bitmask, A2)
			{
				passesPredicates = false;
			}
		}
